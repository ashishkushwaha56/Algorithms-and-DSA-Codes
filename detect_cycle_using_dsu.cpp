...
public:
	...
	void union_(int &a, int &b, int &parent_a, int &parent_b, int parent[], int size_a[]) {
		if (parent_a == parent_b) return ;
		if (size_a[parent_a] >= size_a[parent_b]) {
			size_a[parent_a] += size_a[parent_b];
			parent[parent_b] = parent_a;
		}
		else {
			size_a[parent_b] += size_a[parent_a];
			parent[parent_a] = parent_b;
		}
	}
	int detectCycle(int V, vector<int>adj[])
	{
		int parent[10001], size_a[10001];
		for (int i = 0; i < V; i++) {
			parent[i] = i;
			size_a[i] = 1;
		}
		...
		for (int i = 0; i < V; i++) {
			for (auto &it : adj[i]) {
				int a = i, b = it;
				int u = find_parent(a, parent), v = find_parent(b, parent);
				s = to_string(a) + "-" + to_string(b);
				f = to_string(b) + "-" + to_string(a);
				if (u == v and mb.find(s) == mb.end() and mb.find(f) == mb.end()) {
					ok = true;
					break;
				}
				union_(a, b, u, v, parent, size_a);
				mb.insert(s);
			}
			if (ok) break;
		}
